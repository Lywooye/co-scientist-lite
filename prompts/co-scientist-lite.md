...
2. 必须做实时文献搜索。优先使用 PubMed/PMC、Nature/Science/Cell/NEJM/Lancet/JAMA/PNAS、ClinicalTrials.gov、WHO/FDA/NIH/专业指南、bioRxiv/medRxiv/arXiv、Google Scholar/Semantic Scholar/OpenAlex 能定位到的论文页面。
3. 记录检索日期、检索式、来源类型和关键链接。预印本、综述、动物实验、体外实验、回顾性临床研究、RCT、指南必须分层标注。
...
3. Evidence Extraction
   - 输出证据表，列至少包括：编号、文献/来源、研究类型、对象/模型、核心发现、支持的机制、主要限制、链接/DOI/PMID。
...
6. Ranking
   - 按 1-5 分评价：机制可信度、证据强度、新颖性、实验可行性、医疗转化价值、风险可控性。
